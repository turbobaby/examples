...
const int MAX_USERNAME_CHARS = 16;
 ...
 */
void cleanBoth(char userNameTable[][MAX_USERNAME_CHARS], int users[], char userName[], int socket)
{
	unsigned int n, val = 0, size = strlen(userName);
	strcpy(userName, userNameTable[(7*socket + 11) % 199]);
	memset(userNameTable[(7*socket + 11) % 199], 0, MAX_USERNAME_CHARS);
 ...
 */
void getUsersConnected(char userNameTable[][MAX_USERNAME_CHARS], char buffer[])
{
	bzero(buffer, 0);
	char temp[8];
...
int setPeerSock(int users[], char userName[], int peersock)
{
	...
	for(n = 0; n < size; n++) {
		val += (unsigned char)userName[n] * (n+1);
	}
	i = (7*val + 11) % 199;
 ...
 */
int setName(char userNameTable[][MAX_USERNAME_CHARS], int peersock, char userName[])
{
	int i = (7*peersock + 11) % 199;
	if( userNameTable[i][0] != 0) {
		return 0;
	}
 ...
 */
char* getName(char userNameTable[][MAX_USERNAME_CHARS], char userName[], int peersock)
{
	bzero(userName, 0);
...
int main(int argc, char *argv[])
{
	...
	int       maxfd;
	int       portno;
	int       srvsock, peersock;
	int       peerReq, result, result1, tempPeerReq;
	...
	char userName[MAX_USERNAME_CHARS], senderName[MAX_USERNAME_CHARS];
	char tempPlaceHolder[20];
	int users[199] = { [0 ... 198] = -1};
	char userNameTable[MAX_USERS][MAX_USERNAME_CHARS];
	memset(userNameTable, 0, sizeof(char) * MAX_USERS * MAX_USERNAME_CHARS);
	/*Users table*/

	if (argc == 1 || argc == 2) {
		if (argc == 1) {
			portno = 5555; /* default value */
		} else {
			portno = atoi(argv[1]);
			if (portno < 1024 || portno > 65535) {
				fprintf(stderr,"Wrong portno %d: 1024~65535\n",
					argv[1]);
				return 1;
			}
		}
	...
	do {
		// tempset will be changing as readset gets new users
		// readset has its flags unset
		...
		if (result < 0 && errno != EINTR) {
			...
			if (FD_ISSET(srvsock, &tempset)) {	
				...
				peersock = accept(srvsock, (struct sockaddr*)&clt_addr, &addrlen);				  /*** NEW CONNECTION ***/
					...
					if(!insertSock) {
						...
						printf("NEW USER: %s\n", userName);
						// put name in a table
						setName(userNameTable, peersock, userName);
						...
						for (tempPeerReq = 0; tempPeerReq < MAX_USERS; tempPeerReq++) {
							if (users[tempPeerReq] == -1 || users[tempPeerReq] == peersock)
								continue;
			...
			*/
			for (peerReq = 0; peerReq < maxfd+1; peerReq++) {
				...
				if(result > 0) {
					...
					memset(distributeMSG, 0, sizeof(distributeMSG));
